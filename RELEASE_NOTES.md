...
- 完善租户、建筑和分组级用户的数据范围隔离。
- 设备查询、控制、情景模式和定时任务统一执行分级权限校验。
- admin 保留全局管理权限，其他用户只能管理所属范围内的设备。

### 温控器解析规则

- 按温控器 V5.50 协议更新 `runOn`、运行状态和风机状态解析。
- 区分设定风速与实际运行风速，修正自动风速显示。
- 完善网关子设备的状态对应、ICCID 查询与保存流程。
