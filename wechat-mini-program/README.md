...
上述草稿接口是本方案建议，当前 v2.2 尚未实现。

### 10.6 拍照操作

1. 处理人进入“记录进展”或“解决工单”。
2. 选择拍照或从相册选择。
3. 客户端压缩并逐张上传，显示每张图片进度和失败重试。
4. 上传完成后允许删除或继续添加，总数不超过 10 张。
5. 用户填写处理说明。
...
设备列表接口的集合字段目前不完全统一，例如开关返回 `data.devices`，分散空调返回 `data.list`。API 层应分别适配为统一的客户端分页结构，页面层不要直接判断多种返回字段。
...
1. 是否采用本方案推荐的微信原生 TypeScript。
2. 正式域名、HTTPS 证书和微信小程序 AppID。
3. 是否同意增加工单照片草稿接口，解决多图上传与单次流程动作的衔接。
4. 工单现场定位是否必填；本方案默认选填。
5. 第一版是否只做单设备控制；本方案把情景、定时、批量控制和策略编辑放到第二期。
6. 是否需要第一期同步实现微信账号绑定；本方案默认继续使用现有账号密码。
7. 是否同意按第 10.3 节目标规则补齐服务端工单动作权限；本方案将其列为 P0 安全项。
