...
Page({
  ...
  async loadDevices(reset: boolean) {
    ...
    try {
      const result = await airConditionerApi.getList({
        ...this.data.filters,
  ...
  async togglePower(event: WechatMiniprogram.TouchEvent) {
    ...
    try {
      await airConditionerApi.control(device.routeId, {
        action: "set_power",
        power_state: next ? 1 : 0
      });
  ...
  openDetail(event: WechatMiniprogram.TouchEvent) {
    ...
    wx.navigateTo({ url: `/pages/air-conditioner/detail?${query}` });
  }
});
