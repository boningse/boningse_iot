...
try {
   ...
   * 将照明电气数据保存到分离的电气数据表中
   */
  async saveLightingElectricalDataToTable(device, electricalData, manufacturerCode, useMonthlyPartition = true) {
    try {
      // 检查设备是否有IMEI
      if (!device.imei) {
        const noImeiResult = {
      ...
      let insertMethod = '';
      let tenantCode = null;
      const resolvedManufacturerCode = manufacturerCode || device.manufacturer_code || 'BNDK';
      await this.saveSwitchElectricalData(device, electricalData, resolvedManufacturerCode);

      // 获取租户编码（用于按租户分表）
      if (useMonthlyPartition) {
        ...
        tenantCode = tenantResult.rows[0].code;
      }

      // 根据参数决定使用按月分表还是原始表进行插入
      if (useMonthlyPartition) {
        try {
          // 添加调试日志检查参数值
          logger.debug('调用ensure_tenant_lighting_electrical_tables函数', {
            resolvedManufacturerCode,
            tenantCode,
            manufacturerCodeType: typeof resolvedManufacturerCode,
            tenantCodeType: typeof tenantCode,
            deviceId: device.id,
          ...
          // 确保按租户分表存在
          await pool.query(
            'SELECT ensure_tenant_lighting_electrical_tables($1::VARCHAR, $2::VARCHAR)',
            [resolvedManufacturerCode, tenantCode]
          );

          // 使用按租户分表插入电气数据（直接插入到表中，不使用函数）
          const tableName = `lighting_electrical_${resolvedManufacturerCode.toLowerCase()}_${tenantCode}`;
          query = `
            INSERT INTO ${tableName} (
              device_id, imei, voltage, current, power, energy, created_at, updated_at
            ) VALUES (
              $1, $2, $3, $4, $5, $6, $7, $7
          ...
          values = [
            device.id,
            device.imei,
            electricalData.voltage || 0.000,
            electricalData.current || 0.000,
            electricalData.power || 0.000,
            electricalData.energy || 0.000,
            new Date()
          ];

          result = await pool.query(query, values);
          insertMethod = 'tenant_partition';
        } catch (tenantError) {
          logger.warn('按租户分表插入失败，回退到原始表', {
          ...
          // 回退到原始表插入
          query = `
            SELECT insert_lighting_electrical_data(
              $1::VARCHAR(20),     -- manufacturer_code
              $2::VARCHAR(100),    -- device_imei
              $3::DECIMAL(10,3),   -- device_voltage
              $4::DECIMAL(10,3),   -- device_current
              $5::DECIMAL(10,3),   -- device_power
              $6::DECIMAL(10,3),   -- device_energy
              $7::TIMESTAMP WITH TIME ZONE  -- data_timestamp
            ) as result;
          `;
          ...
          result = await pool.query(query, values);
          insertMethod = 'original_table_fallback';
        }
      } else {
        // 直接使用原始表插入
        query = `
      ...
      if (result.rows[0]?.result) {
        const successResult = {
          ...
          insertMethod,
          useMonthlyPartition,
          tableName: insertMethod === 'tenant_partition' ?
            `lighting_electrical_${resolvedManufacturerCode.toLowerCase()}_${tenantCode}` :
            `lighting_electrical_${resolvedManufacturerCode.toLowerCase()}`,
          data: {
            voltage: electricalData.voltage,
            current: electricalData.current,
            power: electricalData.power,
        ...
        logger.debug('照明电气数据已成功保存', successResult);
        ...
        logger.warn('照明电气数据保存失败', failureResult);
