...
class ThermostatService {
  ...
  async getGroups(tenantId) {
    const query = `
      SELECT 
        ...
        tg.created_at,
        tg.updated_at,
        COUNT(tp.device_id) as device_count
      FROM thermostat_groups tg
      LEFT JOIN thermostat_properties tp ON tg.id = tp.group_id
      WHERE tg.tenant_id = $1
      GROUP BY tg.id, tg.name, tg.description, tg.created_at, tg.updated_at
  ...
  async createGroup(groupData, tenantId) {
    const query = `
      INSERT INTO thermostat_groups (name, description, tenant_id)
      VALUES ($1, $2, $3)
      RETURNING *
    `;

    try {
      const result = await db.query(query, [groupData.name, groupData.description, tenantId]);
   ...
   * 更新分组
   */
  async updateGroup(groupId, groupData, tenantId) {
    const query = `
      UPDATE thermostat_groups 
    ...
    try {
      const result = await db.query(query, [groupData.name, groupData.description, groupId, tenantId]);
  ...
  async deleteGroup(groupId, tenantId) {
    return await db.transaction(async (client) => {
      ...
      // 检查分组是否存在
      const checkQuery = `
        SELECT id FROM thermostat_groups 
        WHERE id = $1 AND tenant_id = $2
      `;
      const checkResult = await client.query(checkQuery, [groupId, tenantId]);
      ...
      // 删除分组
      await client.query('DELETE FROM thermostat_groups WHERE id = $1', [groupId]);
   ...
   * 将设备添加到分组
   */
  async addDeviceToGroup(groupId, deviceIds, tenantId) {
    return await db.transaction(async (client) => {

      // 检查分组是否存在
      const groupCheck = await client.query(
        'SELECT id FROM thermostat_groups WHERE id = $1 AND tenant_id = $2',
        [groupId, tenantId]
      );
      ...
      const result = await client.query(updateQuery, [groupId, deviceIds, tenantId]);
   ...
   * 从分组中移除设备
   */
  async removeDeviceFromGroup(groupId, deviceId, tenantId) {
    const query = `
      UPDATE thermostat_properties 
    ...
    try {
      const result = await db.query(query, [deviceId, groupId, tenantId]);
      return result.rowCount > 0;
    } catch (error) {
      logger.error('从分组中移除设备失败:', error);
      throw error;
    }
  }
