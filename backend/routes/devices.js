...
router.get('/tree', authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin') {
      filterTenantId = req.user.tenant_id;
    } else if (tenantId) {
    ...
    const query = `SELECT * FROM get_device_tree($1)`;
    ...
    results.forEach(device => {
      if (device.parent_device_id) {
        ...
        if (parent) {
          parent.children.push(deviceMap.get(device.id));
        }
      } else {
        rootDevices.push(deviceMap.get(device.id));
      }
    });
...
router.get('/gateways', authenticateToken, async (req, res) => {
  try {
    ...
    const gateways = await Device.findAll({
      where,
      attributes: ['id', 'name', 'device_id', 'imei', 'status', 'location'],
      include: [
        {
          model: DeviceType,
          as: 'device_type',
...
router.get('/:id/children', authenticateToken, async (req, res) => {
  try {
    ...
    // 首先检查父设备是否存在且有权限访问
    const parentDevice = await Device.findByPk(id);
    if (!parentDevice) {
      return res.status(404).json({
        success: false,
        message: '父设备不存在'
    ...
    // 权限检查
    if (req.user.role !== 'admin' && parentDevice.tenant_id !== req.user.tenant_id) {
      return res.status(403).json({
        success: false,
        message: '无权访问该设备'
      });
    ...
    res.json({
      ...
      data: {
        parent: {
          ...
          device_category: parentDevice.device_category
        },
        children: rows,
        pagination: {
          total: count,
 ...
 */
router.post('/:id/children/batch', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    for (let i = 0; i < devices.length; i++) {
      try {
        const deviceData = {
          ...
          device_category: 'sub_device',
          parent_device_id: id,
          tenant_id: parentDevice.tenant_id,
          created_by: req.user.id
        };
        ...
        if (deviceData.sub_device_sequence) {
          ...
          if (existingSubDevice) {
            errors.push({
              index: i,
              device: devices[i],
              error: `子设备序号 ${deviceData.sub_device_sequence} 在该网关下已存在`
            });
            continue;
          }
        }
        ...
        if (deviceData.imei) {
          ...
          if (existingDevice) {
            ...
            // 如果IMEI已存在，检查是否属于父级网关设备
            if (existingDevice.id === parentDevice.id && 
                existingDevice.device_category === 'gateway') {
              // 允许子设备使用与父级网关相同的IMEI
              console.log(`批量创建：子设备允许使用父级网关的IMEI: ${deviceData.imei}`);
            } else {
              // IMEI属于其他设备，不允许使用
              errors.push({
                index: i,
                device: devices[i],
                error: `IMEI已存在，属于其他设备。子设备只能使用其父网关的IMEI (${parentDevice.imei || parentDevice.device_id})`
              });
              continue;
            }
          }
...
router.get('/gateways', authenticateToken, async (req, res) => {
  try {
    ...
    const gateways = await Device.findAll({
      where,
      attributes: ['id', 'name', 'device_id', 'imei', 'status'],
      include: [
        {
          model: Tenant,
          as: 'tenant',
...
router.get('/tree', authenticateToken, async (req, res) => {
  try {
    ...
    const devices = await Device.findAll({
      where,
      attributes: ['id', 'name', 'device_id', 'device_category', 'parent_device_id', 'status'],
      include: [
        {
          model: Tenant,
          as: 'tenant',
      ...
      order: [['device_category', 'ASC'], ['name', 'ASC']]
    });
...
/**
 * 获取设备层级统计信息
 * GET /api/devices/hierarchy/stats
 */
router.get('/hierarchy/stats', authenticateToken, async (req, res) => {
  try {
    const { tenantId } = req.query;
    ...
    // 统计网关设备的子设备数量
    const gatewayStats = await Device.findAll({
      where: {
        ...where,
        device_category: 'gateway'
      },
      attributes: [
        'id',
        'name',
        [require('sequelize').fn('COUNT', require('sequelize').col('children.id')), 'children_count']
      ],
      include: [
        {
          model: Device,
      ...
      group: ['Device.id', 'Device.name']
    });

    const result = {
      category_stats: stats.reduce((acc, stat) => {
        acc[stat.device_category] = parseInt(stat.dataValues.count);
        return acc;
      }, {}),
      gateway_stats: gatewayStats.map(gateway => ({
        id: gateway.id,
        name: gateway.name,
        children_count: parseInt(gateway.dataValues.children_count) || 0
      }))
    };
    ...
    console.error('获取设备层级统计失败:', error);
    res.status(500).json({
      success: false,
      message: '获取设备层级统计失败',
      error: error.message
