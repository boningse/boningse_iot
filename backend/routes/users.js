...
router.put('/change-password', authenticateToken, async (req, res) => {
  try {
    ...
    logger.info('Password changed', { userId, changedAt: new Date() });
...
router.put('/profile', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    const { email, real_name, phone } = req.body;
    ...
    if (real_name !== undefined) updatedProfile.real_name = real_name;
    ...
    logger.info('User profile updated', { userId, updatedAt: new Date() });
